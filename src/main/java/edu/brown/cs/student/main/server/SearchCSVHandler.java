 ...
 * This class is responsible for searching a CSV file.
 */
public class SearchCSVHandler implements Route {
     ...
     *
     * @param loadHandler - Takes in an object from the LoadCSVHandler class (that initially loads the CSV)
     */
    ...
    /**
     * Handles the incoming request to search the loaded CSV file.
     * If no CSV file has been loaded, it returns an error response.
     * Otherwise, it searches the CSV data.
     *
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        if (searchValue != null){
            try {
                ...
                // Performing search
                List<List<String>> results = csvSearch.searchListOfString(this.csvData, searchValue, columnId);
                if (results.isEmpty()) {
     ...
     *
     * @param data - a list of rows (represented as list of strings) from the CSV that contain the search value.
     * @return A JSON string indicating the successful load of the CSV file and the rows containing the search value.
     */
